...
## 9. 노트/자료 확장 기획 (Google Drive 연동) — (2026-09-18, 구현 완료)

> 캔버스 시안(9.5) 컨펌 후 실제 코드로 반영 완료. 자세한 구현 메모는 HANDOFF.md 20번 참고.
...
### 9.5 화면/UX 설계 (캔버스 시안 v2 — 확정, 코드 반영 완료)
...
속성 타입은 최소 두 가지를 구분해서 렌더링한다: **링크형**(URL처럼 값을 클릭 가능한 링크로,
여러 개면 세로로 나열 — 예: URL)과 **태그형**(옵시디언의 `tags` 속성처럼 값을 칩/배지로 나열 +
칩 끝에 점선 "+" 버튼으로 태그 추가 — DESIGN.md 4번 규칙에 따라 옵시디언처럼 pill(`rounded-full`)이
아니라 **작은 사각 칩(`rounded-sm`)**으로 그림, 이 앱의 배지 스타일과 통일). "속성 추가" 클릭 시
기본으로 태그 속성이 추가되는 것으로 시안에 반영(다른 타입 속성 추가는 나중에 필요해지면 선택
메뉴로 확장).

**코드 반영 시 결정된 것**: 스크랩 승격은 전용 엔드포인트 `POST /api/drive/promote`로 구현
(폴더 확보 + 마크다운 파일 생성 + 원본 스크랩 삭제를 서버에서 한 번에 처리). frontmatter
파싱/직렬화는 별도 라이브러리 없이 `src/lib/frontmatter.ts`에 손으로 최소 구현(모든 속성 값은
항상 YAML 리스트로 쓰고, 읽을 때 값이 전부 `http(s)://`로 시작하면 링크형, 아니면 태그형으로
추론). 자세한 내용은 HANDOFF.md 20번 참고.
...
### 9.7 진행 순서 — 전부 완료 (2026-09-18)

1. ✅ Google Cloud 프로젝트/OAuth 클라이언트 등록 + 앱 안 "Google Drive 연결" 버튼으로 실제 연결
2. ✅ `supabase/schema.sql`에 `drive_folder_id` 컬럼 추가 (projects/areas/resources 3개 테이블) +
   `google_accounts` 테이블(사용자별 refresh token, RLS)
3. ✅ 서버 API route: OAuth 연결 플로우 + 폴더 생성/조회, 파일 목록, 업로드, 마크다운 읽기/쓰기,
   스크랩 승격(`/api/drive/promote`) (9.6, 9.5 참고)
4. ✅ 컨테이너 생성 플로우에 Drive 폴더 자동 생성 연결 — **lazy 생성**으로 결정(9.8 열린 질문 4번):
   컨테이너 상세 화면에서 "자료" 탭을 처음 열 때(또는 스크랩을 처음 승격할 때) `drive_folder_id`가
   없으면 그 자리에서 `/api/drive/folder`를 호출해 만들고 저장함.
5. ✅ 상세 화면 자료 탭 UI 반영 — 파일 목록 + 업로드 버튼 + 새 노트 + 인앱 마크다운 에디터(Properties
   블록 포함), Tasks 탭 안 스크랩 섹션 + 승격 플로우까지 캔버스 시안(9.5) 그대로 코드로 옮김.
6. ⬜ 기존에 이미 만들어둔 프로젝트/영역/리소스(폴더 미생성 상태)는 4번의 lazy 생성으로 자연스럽게
   커버됨 — 별도 마이그레이션 스크립트는 필요 없어짐(9.8 열린 질문 4번 최종 결론).

### 9.8 열린 질문

1. **파일 목록 캐싱 여부**: 매번 Drive API를 실시간 호출할지, 아니면 목록을 DB에 캐시해두고
   주기적/이벤트 기반으로 갱신할지 — 초기엔 실시간 호출로 시작하고 느려지면 캐싱 고려. (미해결,
   현재 구현은 실시간 호출)
2. **마크다운 인앱 에디터 범위**: 문법 하이라이팅/미리보기까지 넣을지, 순수 textarea로 시작할지 —
   **순수 textarea로 시작**(구현 완료). 필요해지면 나중에 확장.
3. **업로드 용량/형식 제한**: 특별히 제한을 둘지, Drive API 기본 한계(파일당 5TB 등)를 그대로
   따를지 — 실사용 패턴 보고 결정. (미해결, 현재는 별도 제한 없음)
4. **기존 컨테이너 소급 처리**: **lazy 생성으로 확정** — 위 9.7 4번 참고, 별도 마이그레이션 불필요.
